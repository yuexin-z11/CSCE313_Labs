...
int main (int argc, char *argv[]) {
    ...
    while(lo == false ){
        ...
        file_req->offset = msg * z; // set offset in the file 
        file_req->length = (msg < filesize) ? msg : filesize;
 // set the ength, watch the last segment

        // send the request buf2
        chan.cwrite(buf2, len); 
        chan.cread(&req_file, file_req->length); 
        file2.write(req_file, len);
