...
int main (int argc, char *argv[]) {
	...
	int e1, e2;
	string filename = "";
	int z = MAX_MESSAGE; // default value for buffer capacity 
	bool new_chan = false;
	...
	while ((opt = getopt(argc, argv, "p:t:e:f:m:c")) != -1) { // collon is used when flag have a value associated with it
		switch (opt) {
			...
			case 'f':
				...
				break;
			case 'm':
				z = atoi(optarg); // convert optarg to an integer
				break;
			case 'c':
				new_chan = true;
				break;
	...
	if (pid < 0){
		...
		while(counter != 0 ){
			...
			file_req->offset = min(m, filesize); // set offset in the file 
			file_req->length = min(m, filesize); // set the ength, watch the last segment
